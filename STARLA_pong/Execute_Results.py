...
parser.add_argument("--s-number", type=int, default=8)
parser.add_argument("--epsilon", type=int, default=30)
parser.add_argument("--abstract-level", type=float, default=0.5)
...
def save_all_data(pop, no_of_Objectives, threshold_criteria, stored_data):
    ...
    '''
    threshold_criteria_to_add_to_archive = [400, 0.06, 0.05, 0.05]
...
env2 = PLEStoreWrapper(PLE(TestPong(width=64*8, height=48*8, MAX_SCORE=1), display_screen=False))
...
def re_execute_final_ff_v1_woprint(model, env, candidate):
    ...
    if is_fail_state(obs):
        ff = True
    assert done, "not finished in 1000 steps "
    return differences, divs, ff
...
for d in [5]:
    ...
    for generations in items:
        if generations == 'res' or generations == '.ipynb_checkpoints':
            continue
        # if generations == f"May17_generations_r110_rt400_population1500lastfull_run2_{s_number}.pickle":
        if generations[:56] == f"May17_generations_r110_rt400_population1500lastfull_run4":
            arch2 = []
            stat = []
            ft = []
            print("\n\n-----------------------------------------------------\n\n")
            ...
            for i in range(len(data)):
                ...
                for ind_ in data[i]:
                    ...
                    if fitness_reward(value_) < 400:
                        rewardfault.append(ind_)
                    if is_fail_state(last_state[0]):
                        functionalfault.append(ind_)
                    if (fitness_reward(value_) > 400 and not is_fail_state(last_state[0])):
